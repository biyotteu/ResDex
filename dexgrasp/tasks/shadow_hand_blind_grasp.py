...
# This file is a modified version of shadow_hand_grasp.py.
# The difference is that we only use 
...
class ShadowHandBlindGrasp(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        # take dof_pos, hand_pose, action, object_pose and goal pose as observation
        num_obs = 147 + 6 
        self.num_obs_dict = {
            "full_state": num_obs
        }
        self.num_hand_obs = 147
        self.up_axis = 'z'
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            # print("env: ", i, "object_code: ", object_code)
            available_scale = []
            ...
            # save the environment's index with its corresponding object's id and scale
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        now_position = 0
        # 0:66
        self.obs_buf[:, now_position:now_position+self.num_shadow_hand_dofs] = unscale(self.shadow_hand_dof_pos,
                                                self.shadow_hand_dof_lower_limits,
                                                self.shadow_hand_dof_upper_limits)
        now_position += self.num_shadow_hand_dofs

        # self.obs_buf[:,now_position:2 * self.num_shadow_hand_dofs] = self.vel_obs_scale * self.shadow_hand_dof_vel
        # now_position += self.num_shadow_hand_dofs

        # As force has a big sim2real gap, we do not use it in the observation
        # self.obs_buf[:,now_position:3 * self.num_shadow_hand_dofs] = self.force_torque_obs_scale * self.dof_force_tensor[:, :24]
        # now_position += self.num_shadow_hand_dofs
        ...
        # # 66:131: ft states
        self.obs_buf[:, now_position:now_position + num_ft_states] = aux
        now_position += num_ft_states

        # # 131:161: ft sensors: do not need repose
        self.obs_buf[:, now_position:now_position+ num_ft_force_torques] = self.force_torque_obs_scale * self.vec_sensor_tensor[:, :30]
        now_position += num_ft_force_torques


        # hand_pose_start = fingertip_obs_start + 95
        # # 161:167: hand_pose, palm
        self.obs_buf[:, now_position:now_position + 3] = self.unpose_point(self.right_hand_pos)
        euler_xyz = get_euler_xyz(self.unpose_quat(self.hand_orientations[self.hand_indices, :]))
        self.obs_buf[:, now_position + 3:now_position + 4] = euler_xyz[0].unsqueeze(-1)
        self.obs_buf[:, now_position + 4:now_position + 5] = euler_xyz[1].unsqueeze(-1)
        self.obs_buf[:, now_position + 5:now_position + 6] = euler_xyz[2].unsqueeze(-1)
        now_position += 6
        ...
        self.obs_buf[:, now_position:now_position + 24] = aux
        now_position += 24


        
        # now_position = action_obs_start + 24  # 144
        # 191:207 object_pose, goal_pos
        self.obs_buf[:, now_position:now_position + 3] = self.unpose_point(self.object_pose[:, 0:3])
        now_position += 3
        # self.obs_buf[:, now_position:now_position + 4] = self.unpose_quat(self.object_pose[:, 3:7])
        # now_position += 4
        # self.obs_buf[:, now_position + 7:now_position + 10] = self.unpose_vec(self.object_linvel)
        # self.obs_buf[:, now_position + 10:now_position + 13] = self.vel_obs_scale * self.unpose_vec(self.object_angvel)
        self.obs_buf[:, now_position:now_position + 3 ] = self.unpose_vec(self.goal_pos - self.object_pos)
        now_position += 3

         # 207:236 goal
        # hand_goal_start = obj_obs_start + 16
        # self.obs_buf[:, now_position:now_position + 3] = self.delta_target_hand_pos
        # self.obs_buf[:, now_position + 3:now_position + 7] = self.delta_target_hand_rot
        # self.obs_buf[:, now_position + 7:now_position + 29] = self.delta_qpos
        # now_position += 29
        ...
        # self.obs_buf[:, now_position:now_position + 64] = 0.1 * self.visual_feat_buf
        # now_position += 64
...
def compute_hand_reward(
    ...
    # # for some strange object
    # change the object_pos and object_handle_pos to add a float bias (0.06, 0, 0)
    # bias = to_torch([0.06, 0, 0], device=object_pos.device).repeat(object_pos.shape[0], 1)
    # object_pos = object_pos + bias
    # object_handle_pos = object_handle_pos + bias
    ...
    # perhaps only use qpos can be more generalizable
    #delta_value = 0.1 * delta_qpos_value
    #target_flag = (delta_qpos_value <= 6.0).int()
    ...
    if goal_cond:
        ...
        #goal_hand_rew = torch.where(flag == 3, 1 * (0.9 - 2 * goal_dist), goal_hand_rew)
        ...
        # new reward item, make fingertip not close to the palm
        # create a normalized vector from palm to the object
        obj2palm = right_hand_pos - object_handle_pos
        obj2palm = obj2palm / torch.norm(obj2palm, p=2, dim=-1, keepdim=True)
        # create a normalized vector from five fingertips to the  object
        obj2ff = right_hand_ff_pos - object_handle_pos
        obj2ff = obj2ff / torch.norm(obj2ff, p=2, dim=-1, keepdim=True)
        obj2mf = right_hand_mf_pos - object_handle_pos
        obj2mf = obj2mf / torch.norm(obj2mf, p=2, dim=-1, keepdim=True)
        obj2rf = right_hand_rf_pos - object_handle_pos
        obj2rf = obj2rf / torch.norm(obj2rf, p=2, dim=-1, keepdim=True)
        obj2lf = right_hand_lf_pos - object_handle_pos
        obj2lf = obj2lf / torch.norm(obj2lf, p=2, dim=-1, keepdim=True)
        obj2th = right_hand_th_pos - object_handle_pos
        obj2th = obj2th / torch.norm(obj2th, p=2, dim=-1, keepdim=True)
        
        # dot product of the normalized vectors
        ff_dot = torch.sum(obj2palm * obj2ff, dim=-1)
        mf_dot = torch.sum(obj2palm * obj2mf, dim=-1)
        rf_dot = torch.sum(obj2palm * obj2rf, dim=-1)
        lf_dot = torch.sum(obj2palm * obj2lf, dim=-1)
        th_dot = torch.sum(obj2palm * obj2th, dim=-1)

        # reward item
        # may be thumb needs more freedom
        ft_palm_angle = ff_dot + mf_dot + rf_dot + lf_dot + th_dot

        # # ensure that palm_to_object is vertical to the ground
        # # first create a vertical vector to the ground
        # vertical = torch.tensor([0.0, 0.0, -1.0], device=right_hand_pos.device).repeat(right_hand_pos.size(0), 1)
        # # calculate the angle between palm_to_object and vertical
        # palm2obj = -obj2palm
        # palm_dot = torch.sum(palm2obj * vertical, dim=-1)
        ...
        # reward for thin object
        th_ff_dot = torch.sum(obj2ff * obj2th, dim=-1)
        th_mf_dot = torch.sum(obj2mf * obj2th, dim=-1)
        th_rf_dot = torch.sum(obj2rf * obj2th, dim=-1)
        th_lf_dot = torch.sum(obj2lf * obj2th, dim=-1)
        th_4_finger_angle = th_ff_dot + th_mf_dot + 0.5* th_rf_dot + 0.1* th_lf_dot

        reward = -0.05 * th_4_finger_angle - 0.5 * right_hand_finger_dist - 1.0 * right_hand_dist + goal_hand_rew + hand_up + bonus




        # reward for big item
        # maybe the coefficient is the key (original one is -0.1)
        # reward = -0.1 * ft_palm_angle - 0.5 * right_hand_finger_dist - 1.0 * right_hand_dist + goal_hand_rew + hand_up + bonus

        # reward for small item
        # reward = -0.2 * th_dot -0.5 * right_hand_finger_dist - 1.0 * right_hand_dist + goal_hand_rew + hand_up + bonus

        # reward regulate the thumb and the index finger
        # reward = -0.1 * (ff_dot + th_dot) - 0.5 * right_hand_finger_dist - 1.0 * right_hand_dist + goal_hand_rew + hand_up + bonus
        # print every part of the reward
        # print("angle: ", -2.0 * ft_palm_angle)
