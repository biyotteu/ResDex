...
class ShadowHandResidualGrasp(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        self.eval_min_hold_time_sec = float(self.cfg.get("env", {}).get("evalMinHoldTimeSec", 1.0))
        self._init_eval_metric_buffers()

        if self.viewer != None:
            cam_pos = gymapi.Vec3(0.5, -0.75, 1.25)
    ...
    def _init_eval_metric_buffers(self):
        ...
        # aggregated over episodes (e.g., 200 eval episodes)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # # save the visual feat
        # save_dir="Intermediate_Data/"
        # torch.save(self.visual_feat_data, save_dir + "visual_feat_data.pt")
        # torch.save(self.object_code_list, save_dir + "object_code_list.pt")
        # torch.save(self.object_scale_id_list, save_dir + "object_scale_id_list.pt")
        # torch.save(self.id2scale, save_dir + "id2scale.pt")
    ...
    def compute_observations(self):
        ...
        self.ft_cf = self.net_cf[:, self.fingertip_handles, 0:3]
        # convert the fingertip contact force to 0-1 contact
        self.contact_tensor = (self.ft_cf.abs().sum(dim=-1) > 0).int()
        # print(self.contact_tensor)
        
        # print("ft_cf: ", self.ft_cf[0])
        # print("ff_cf: ", self.right_hand_ff_cf[0])
        # print("mf_cf: ", self.right_hand_mf_cf[0])
        # print("rf_cf: ", self.right_hand_rf_cf[0])
        # print("lf_cf: ", self.right_hand_lf_cf[0])
        # print("th_cf: ", self.right_hand_th_cf[0])
        # exit()
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        # self.records.append(self.vec_sensor_tensor[0, :30].clone())
        ...
        now_position += 4
        self.obs_buf[:, now_position:now_position + 3] = self.unpose_vec(self.object_linvel)
        now_position += 3
        self.obs_buf[:, now_position :now_position + 3] = self.vel_obs_scale * self.unpose_vec(self.object_angvel)
        now_position += 3
        # now_position += 16
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        if self.random_prior:
            for env_id in env_ids:
                ...
                # print(data['target_qpos'][prior_idx])
                # print(data['target_hand_pos'][prior_idx])
                # print(data['target_hand_rot'][prior_idx])
                # print(data['object_euler_xy'][prior_idx])
                # print(data['object_init_z'][prior_idx])
                # exit()
        ...
        # -----------------------------------------------------------------
        # Randomize hand start pose:
        # - approach direction: sample from {top, side_x, side_y, diag, ...}
        # - yaw stays aligned to prior_rot_z (keeps difficulty manageable)
...
def compute_hand_reward(
    ...
    if goal_cond:
        ...
        reward = goal_hand_rew + hand_up + bonus - 0.5 * delta_value
        # may be I should lower the coef of delta_value
        # reward =goal_hand_rew + hand_up + bonus - 0.1* delta_value
        ...
        # reward for hyper policy
        # seems very efficient
        # not perfect
        reward =  goal_hand_rew + hand_up + bonus 

        # try a gradual reward
        # reward = bonus
        # add right_hand_dist
        # reward = - 1.0 * right_hand_dist + goal_hand_rew + hand_up + bonus
        # add finger_dist
        # reward = -0.5 * right_hand_finger_dist - 1.0 * right_hand_dist + goal_hand_rew + hand_up + bonus
        
        # change it to a sparse reward, we only give reward when the grasping is successful
        # if it onece success and the object keep on the top of the goal, wogive reward
        # sparse_reward = torch.where(flag==2, torch.ones_like(right_hand_finger_dist), torch.zeros_like(right_hand_finger_dist))
        # sparse_reward = torch.where(lowest >= 0.630, sparse_reward, torch.zeros_like(sparse_reward))
        # sparse_reward = torch.where(successes==1, sparse_reward, torch.zeros_like(sparse_reward))
    ...
    # reward is 1 if success, otherwise is -1
    # reward = torch.where(successes == 1, torch.ones_like(successes), -torch.ones_like(successes))
