...
from algorithms.rl.residual import RolloutStorage
from algorithms.rl.residual import ActorCritic
...
class Residual:
    ...
    def __init__(self,
                 ...
                 asymmetric=False,
                 is_vision = False,
                 base_model_list_dir=None,
                 base_obs_num=0,
                 residual_obs_num=0,
                 save_test_traj=False,
                 ):
        
        # asert is_testing and save_test_traj have the same value
        if save_test_traj:
            assert is_testing
        ...
        self.step_size = learning_rate
         ...
         # base models that are fixed
        assert base_model_list_dir is not None
        # load the base model list from the directory
        if base_model_list_dir == '':
            self.base_models =[]
        else:
            # yaml load
            base_model_list = yaml.load(open(base_model_list_dir, 'r'), Loader=yaml.FullLoader)["base_model_list"]
            
            # with open(base_model_list_dir, 'r') as f:
            #     base_model_list = json.load(f)
            self.base_models=[]
            for base_model_dir in base_model_list:
                base_model = actor_critic_class(self.base_obs_space,  self.state_space.shape, self.action_space.shape,
        ...
        self.residual_action_shape = [len(self.base_models)+24] 
        self.num_coef = len(self.base_models)
        self.softmax = nn.Softmax(dim=-1)

        # PPO components
        self.vec_env = vec_env
        # self.actor_critic = actor_critic_class(self.residual_obs_space, self.state_space.shape, self.action_space.shape,
        #                                        init_noise_std, model_cfg, asymmetric=asymmetric, use_pc = self.is_vision)
        self.actor_critic = actor_critic_class(self.residual_obs_space, self.state_space.shape, self.residual_action_shape,
                                               init_noise_std, model_cfg, asymmetric=asymmetric, use_pc = self.is_vision,
                                               type="residual")
        # add a softmax layer to the output of the actor_critic
        self.actor_critic.to(self.device)
        # self.storage = RolloutStorage(self.vec_env.num_envs, num_transitions_per_env, self.residual_obs_space,
        #                               self.state_space.shape, self.action_space.shape, self.device, sampler)
        self.storage = RolloutStorage(self.vec_env.num_envs, num_transitions_per_env, self.residual_obs_space,
                                        self.state_space.shape, self.residual_action_shape, self.device, sampler)
        ...
        if self.is_testing and  self.save_test_traj:
            ...
            for _ in range(200):
                with torch.no_grad():
                    ...
                    residual_actions_list.append(residual_actions[:, :len(self.base_models)])
                    # residual_actions_list.append(residual_actions)
                ...
                if _ == 200-2:
                    ...
                    data_dict={"residual_actions":residual_actions_list, 
                                 "success_rate":self.vec_env.task.successes,
                                 "object_id_buf":self.vec_env.task.object_id_buf,
                                 "object_code_list":self.vec_env.task.object_code_list,
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    base_actions = self.get_base_actions(self.get_obs(current_obs, model="base")) 
                    residual_obs = self.get_full_residual_obs(self.get_obs(current_obs, model="base"), base_actions, self.get_obs(current_obs, model="residual"))
                    residual_actions, actions_log_prob, values, mu, sigma = self.actor_critic.act(residual_obs, current_states)

                    #  # compute the entropy loss
                    # residual_pdf = self.softmax(residual_actions[:, :len(self.base_models)])
                    # entropy_loss = torch.sum(-residual_pdf * torch.log(residual_pdf), dim=-1)
                    ...
                    self.storage.add_transitions(residual_obs, current_states, residual_actions, rews, dones, values, actions_log_prob, mu, sigma)
                ...
                _, _, last_values, _, _ = self.actor_critic.act(residual_obs, current_states)
                stop = time.time()
    ...
    def compose_actions(self, base_actions, residual_action):
        #actions = torch.zeros_like(base_actions[0])
        actions = torch.zeros(self.vec_env.num_envs, 24).to(self.device)
        coef = residual_action[:, :self.num_coef]
        residual_action = residual_action[:, self.num_coef:]        
    ...
    def get_base_actions(self, obs):
        actions = []
        for base_model in self.base_models:
            actions.append(base_model.act_inference(obs).detach())
        return actions
    
    def get_obs(self, obs, model="base"):
        if model == "base":
            return obs[:, :self.base_obs_space[0]]
        elif model == "residual":
            return obs
        else:
            raise ValueError("Invalid model type")
        
    def get_full_residual_obs(self, base_obs, base_actions, residual_obs):
        
        #print(residual_obs[:, -64:].shape )
       # print(residual_obs[0, -64:])
      #  print(residual_obs[4000, -64:])
        return residual_obs
#       combined_tensor = torch.cat((residual_obs[:, 0:22], residual_obs[:, 66:207], residual_obs[:, 236:300]), dim=1).to(self.device)
